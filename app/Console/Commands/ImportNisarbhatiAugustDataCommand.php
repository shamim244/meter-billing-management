...
class ImportNisarbhatiAugustDataCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:nisarbhati-august 
                            {--email=shamim244d@gmail.com : User email}
                            {--mru=23 : MRU ID (default: 23 for NISARBHATI)}
     ...
     * @var string
     */
    protected $description = 'Import August working readings and statuses for NISARBHATI (0244 / MRU 23) from migration directory';
    ...
    public function handle(): int
    {
        ...
        if (! $mru) {
            $mru = Mru::where('code', '0244')->where('user_id', $user->id)->first();
        }

        if (! $mru) {
            $this->error("MRU not found for ID {$mruId} / code 0244 and user {$user->id}");

            return Command::FAILURE;
        }

        $baseDir = base_path('.agents/docs/Migrate/NISARBHATI-0244/August');
        ...
        $this->info("Starting NISARBHATI (0244) August migration for User #{$user->id} ({$user->email}), MRU #{$mru->id} ({$mru->name})...");
        if ($dryRun) {
