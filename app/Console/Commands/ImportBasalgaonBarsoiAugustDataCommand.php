...
class ImportBasalgaonBarsoiAugustDataCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:basalgaon-barsoi-august 
                            {--email=shamim244d@gmail.com : User email}
                            {--mru=19 : MRU ID (default: 19 for Basalgaon-Barsoi)}
     ...
     * @var string
     */
    protected $description = 'Import August working readings and statuses for Basalgaon-Barsoi (0470 / MRU 19) from migration directory';
    ...
    public function handle(): int
    {
        ...
        if (! $mru) {
            $mru = Mru::where('code', '0470')->where('user_id', $user->id)->first();
        }

        if (! $mru) {
            $this->error("MRU not found for ID {$mruId} / code 0470 and user {$user->id}");

            return Command::FAILURE;
        }

        $baseDir = base_path('.agents/docs/Migrate/Basalgaon-Barsoi-0470');
        ...
        $this->info("Starting Basalgaon-Barsoi (0470) August migration for User #{$user->id} ({$user->email}), MRU #{$mru->id} ({$mru->name})...");
        if ($dryRun) {
